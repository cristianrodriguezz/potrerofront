...
export default function Canvas() {
  ...
  const handleTransformEnd = (id: string, newTransform: {
    xPercent: number;
    yPercent: number;
    scaleRel: number;
    rotation: number;
  ...
  return (
    <div className="space-y-4 p-4 max-w-4xl mx-auto">
      <div className="flex flex-wrap gap-4 items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Diseñá tu escudo</h1>
          <p className="text-gray-600">Escala proporcional al tamaño del escudo</p>
        </div>
        ...
        <div className="flex gap-2">
          {Object.entries(shieldOptions).map(([key, Component]) => (
            <button
              key={key}
              onClick={() => setSelectedShield(key as keyof typeof shieldOptions)}
      ...
      >
        ...
        <div className="absolute bottom-4 left-4 bg-white/80 backdrop-blur-sm px-3 py-2 rounded-lg text-sm text-gray-700 shadow-sm">
          Tamaño: {Math.round(containerSize.width)}×{Math.round(containerSize.height)}px
        </div>
      </div>

      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
        <h3 className="font-medium text-gray-800 mb-2">Sistema de escalado proporcional:</h3>
        <ul className="text-sm text-gray-600 list-disc pl-5 space-y-1">
          <li>Todas las posiciones son porcentajes relativos al escudo</li>
          <li>La escala es relativa al tamaño del escudo</li>
          <li>Si el escudo se achica, los elementos también se achican proporcionalmente</li>
          <li>Consistencia total en cualquier tamaño de pantalla</li>
        </ul>
      </div>
    </div>
  );
